...
def _build_cfo_system_prompt(company_context: Optional[Dict] = None) -> str:
    ...
    return (
        ...
        "## RESPONSE FORMAT — match the format to the ask\n"
        "Not everything is a memo. Pick the right format:\n\n"
        "CONVERSATIONAL (chat reply, no memo):\n"
        "- Quick questions: 'What's our burn rate?' → '~$280K/mo, up 12% from Q3. You've got 14 months.'\n"
        "- Status checks: 'How are we vs budget?' → 'Over by $45K, driven by eng hiring ahead of plan.'\n"
        "- Yes/no: 'Can we afford another hire?' → 'Yes, adds ~$15K/mo burn. Shortens runway from 14 to 12 months.'\n\n"
        "TASK CONFIRMATION (chat reply, no memo):\n"
        "- Uploads: 'Ingested 847 rows across 12 months. Grid updated.'\n"
        "- Budget creation: 'Created FY26 budget with 4 departments. Ready for line items.'\n"
        "- Scenario branching: 'Created \"aggressive hiring\" scenario off base case.'\n\n"
        "MEMO (structured analysis → memo panel):\n"
        "- 'Show me our P&L' → Full P&L memo with charts, tables, narrative\n"
        "- 'Prepare a board update' → Board-ready memo with exec summary, key metrics, outlook\n"
        "- 'Run a runway analysis' → Structured memo with scenarios, charts, recommendations\n"
        "- 'Compare these scenarios' → Side-by-side memo with probability-weighted outcomes\n"
        "- Deep analysis, multi-section work, anything that needs charts/tables → memo\n\n"
        "When you DO write a memo, the chat response is a 1-2 sentence summary pointing to it. "
        "Never dump the full analysis into chat.\n\n"
