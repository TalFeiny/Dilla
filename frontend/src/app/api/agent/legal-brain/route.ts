/**
 * Legal Brain API Route - Proxies to FastAPI Backend
 * Same architecture as cfo-brain but hits the GC agent endpoint.
 * Handles contract review, clause extraction, obligation tracking, and ERP attribution.
 */

import { NextRequest, NextResponse } from 'next/server';
import { saveToSupabase } from '../unified-brain/save-to-supabase';

// Legal agent may run multi-step clause extraction — needs long timeout
...
export async function POST(request: NextRequest) {
  try {
    ...
    const backendEndpoint = body.stream
      ? `${BACKEND_URL}/api/agent/legal-brain-stream`
      : `${BACKEND_URL}/api/agent/legal-brain`;
    ...
    if (!response.ok) {
      ...
      console.error('Legal backend error:', error);
      return NextResponse.json(
        { error: 'Legal backend request failed', details: error, agent: 'legal' },
    ...
    try {
      ...
      console.error('Failed to parse Legal backend JSON:', parseError);
      return NextResponse.json(
        { error: 'Invalid response from Legal backend', details: 'Non-JSON payload', agent: 'legal' },
        { status: 502 }
      );
    }
    ...
    if (backendJson === null || typeof backendJson !== 'object') {
      return NextResponse.json(
        { error: 'Invalid response from Legal backend', details: 'Empty payload', agent: 'legal' },
        { status: 502 }
      );
    }
    ...
    if (!normalizedResponse) {
      return NextResponse.json(
        { error: 'Empty response from Legal backend', agent: 'legal' },
        { status: 500 }
      );
    }
    ...
    if (normalizedResponse.success && normalizedResponse.result) {
      ...
      saveToSupabase(
        ...
        console.error('[legal-brain] Background Supabase save failed:', err?.message || err);
      });
    ...
    console.error('Legal brain route error:', error);
    return NextResponse.json(
      {
