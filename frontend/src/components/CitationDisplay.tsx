...
/** Normalize and deduplicate citations by url (or id+source+title). Preserves first occurrence. */
export function deduplicateCitations(citations: Citation[]): Citation[] {
  const seen = new Set<string>();
  return citations
    .filter((c) => {
      const key = (c.url || String(c.id) || `${c.source}-${c.title || ''}`).trim();
...
export const CitationDisplay: React.FC<CitationDisplayProps> = ({
  ...
  const normalizedCitations = deduplicateCitations(citations);
  ...
  const renderInlineText = (text: string) => {
    ...
    return parts.map((part, index) => {
      ...
      if (!isNaN(citationNum) && normalizedCitations.some(c => (c.number ?? c.id) == citationNum)) {
        return (
          <sup key={index}>
            <a
  ...
  const renderCitation = (citation: Citation, isExpanded: boolean) => (
    <li
      key={citation.number ?? citation.id ?? citation.url ?? citation.source}
      id={`citation-${citation.number ?? citation.id}`}
    ...
    >
      <div className="flex items-start justify-between">
        <div className="flex-1">
          <div className="flex items-center gap-2 mb-2">
            ...
            {citation.url ? (
              ...
              <span className="font-medium text-gray-900">{citation.title || citation.source || 'Source'}</span>
            )}
          </div>
        ...
        <button
          onClick={() => toggleCitation(citation.number ?? (citation.id as number) ?? 0)}
  ...
  const displayCitations = showAllCitations ? normalizedCitations : normalizedCitations.slice(0, 5);
  ...
  return (
    <div className={`citation-display ${className}`}>
      ...
      {format !== 'inline' && normalizedCitations.length > 0 && (
        <div className="citations-bibliography">
          <div className="flex items-center justify-between mb-4">
            <h3 className="text-lg font-semibold text-gray-900 flex items-center gap-2">
              ...
              <span className="text-sm font-normal text-gray-500">
                ({normalizedCitations.length} total)
              </span>
            </h3>
            ...
            {normalizedCitations.length > 5 && (
              ...
              >
                {showAllCitations ? 'Show Less' : `Show All ${normalizedCitations.length} Citations`}
              </button>
            )}
          </div>
          ...
          <ol className="space-y-3">
            {displayCitations.map((citation) => 
              renderCitation(citation, expandedCitations.has(citation.number ?? (citation.id as number) ?? 0))
            )}
          </ol>

          {!showAllCitations && normalizedCitations.length > 5 && (
            <div className="mt-4 text-center">
              ...
              >
                ...
                View {normalizedCitations.length - 5} More Citations
              </button>
            </div>
          )}
        </div>
